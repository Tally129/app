...
def require_roles(*roles):
    async def dep(request: Request, user=Depends(get_current_user)):
        role = user.get("role")
        # Break-glass: "auditor" is a read-only super-role. Allowed on any GET,
        # but every access is stamped with emergency=true in the audit log.
...
async def change_password(payload: PasswordChange, request: Request, user=Depends(get_current_user)):
    ...
    reason = validate_password_strength(payload.new_password, email=user.get("email", ""), full_name=user.get("full_name") or "")
    if reason:
        raise HTTPException(status_code=400, detail=reason)
    await db.users.update_one({"id": user["id"]}, {"$set": {"password_hash": hash_password(payload.new_password), "password_changed_at": datetime.now(timezone.utc)}})
